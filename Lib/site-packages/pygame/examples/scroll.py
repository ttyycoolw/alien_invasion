...
An zoomed images viewer that demonstrates Surface.scroll

This example shows a scrollable images that has a zoom factor of eight.
It uses the Surface.scroll function to shift the images on the display
surface. A clip rectangle protects a margin area. If called as a function,
the example accepts an optional images file path. If run as a program
it takes an optional file path command line argument. If no file
is provided a default images file is used.
...
def main(image_file=None):
    ...
    try:
        ...
        if image_w < view_size[0] or image_h < view_size[1]:
            print("The source images is too small for this example.")
            print("A %i by %i or larger images is required." % zoom_view_size)
            return
